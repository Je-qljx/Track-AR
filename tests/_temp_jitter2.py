...
# Use LaneFeatureTracker directly
...
for fi in range(50):
    ...
    if fi > 0:
        H_info.append((fi, mi.get('method','?'), mi.get('klt_tracked',0), mi.get('klt_inliers',0), mi.get('total_pts',0)))
    if fi >= 48:
        break

for fi, method, tracked, inliers, total in H_info:
    print(f'  fi={fi}: method={method}, tracked={tracked}, inliers={inliers}, total={total}')
print(f'Final H: {ft.H_calib_current[0,2]:.2f}, {ft.H_calib_current[1,2]:.2f}')
