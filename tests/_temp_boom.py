...
def perturb_boom(f, fps, r0, t0):
    r = r0.copy()
    t = t0.copy()
    t[1, 0] += 0.3 * math.sin(2*math.pi*f/(fps*8.0))
    return r, t

dm_vals = []
for fi in range(700):
    rvec, tvec = perturb_boom(fi, fps, r0, t0)
    ...
    if mi.get('drift_corrected') and fi <= 300:
        print(f'  fi={fi}: drift corrected, d_m_1={pipeline.assigner.athletes[1].d_m:.3f}')
