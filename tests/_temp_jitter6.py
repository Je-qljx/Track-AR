...
# Check if first_pts was set after calibration
print(f'first_pts after calib: {len(ft._first_pts) if ft._first_pts is not None else None}')
...
for fi in range(65):
    ...
    fp = len(ft._first_pts) if ft._first_pts is not None else -1
    print(f'fi={fi}: method={mi.get("method","?")}, first_pts={fp}, frame_count={ft._frame_count}, drift_corrected={mi.get("drift_corrected",False)}')
