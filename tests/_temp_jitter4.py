...
ft = pipeline.frame_tracker  # use pipeline's tracker
...
for fi in range(850):
    ...
    if mi.get('drift_corrected'):
        corrected += 1
        if corrected <= 3 or fi % 200 == 0:
            print(f'  fi={fi}: drift corrected={mi.get("drift",0):.3f}')
    if fi % 200 == 0:
...
if not pipeline.timer.race_finished:
    ...
    print(f'Total drift corrections: {corrected}')
